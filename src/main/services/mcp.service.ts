...
export class MCPService {
  ...
  private readonly AGW_CLIENT = process.env.AGW_CLIENT
  private readonly AGW_SECRET = process.env.AGW_SECRET
  private readonly AGW_BASE_URL = process.env.AGW_BASE_URL

  private constructor() {
    // 校验阿里云网关配置（仅在使用相关功能时需要）
    if (this.AGW_CLIENT && (!this.AGW_SECRET || !this.AGW_BASE_URL)) {
      console.warn(
        '[MCP] 阿里云网关配置不完整，市场功能可能无法使用。请设置环境变量: AGW_CLIENT, AGW_SECRET, AGW_BASE_URL'
      )
    }
  }
   ...
   * 生成阿里云网关签名
   */
  private async generateAGWSignature(): Promise<{
    client: string
    signature: string
    timestamp: string
  }> {
    if (!this.AGW_CLIENT || !this.AGW_SECRET) {
      throw new Error(
        '阿里云网关配置缺失，请设置环境变量: AGW_CLIENT, AGW_SECRET'
      )
    }

    const timestamp = Date.now().toString()
    const crypto = await import('crypto')
    const signatureData = `${timestamp}\n${this.AGW_SECRET}`
    const signature = crypto.createHash('sha256').update(signatureData).digest('hex')

    return { client: this.AGW_CLIENT, signature, timestamp }
  }

  async getMarketServers(params: {
    empId?: string
    ...
    if (!this.AGW_BASE_URL) {
      throw new Error('阿里云网关配置缺失，请设置环境变量: AGW_BASE_URL')
    }

    try {
      const { client, signature, timestamp } = await this.generateAGWSignature()

      const queryParams = new URLSearchParams({
        client_id: 'sui-kit',
        'AGW-Client': client,
        'AGW-Signature': signature,
        'AGW-Timestamp': timestamp,
        order_by: params.orderBy || 'USAGE',
        page: String(params.page || 1),
        pageSize: String(params.pageSize || 10)
      })

      if (params.empId) {
        queryParams.append('emp_id', params.empId)
      }

      const url = `${this.AGW_BASE_URL}/servers?${queryParams.toString()}`
  ...
  async getServerDetail(serverName: string): Promise<{
    name: string
    displayName: string
    description?: string
    avatarUrl?: string
    ...
    try {
      ...
      const url = `${this.AGW_BASE_URL}/servers/${serverName}?${queryParams.toString()}`
