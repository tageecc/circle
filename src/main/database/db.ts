...
class CircleDatabase {
  ...
  private migrateSessionsTable(): void {
    try {
      // Check if sessions table exists and has user_id column
      const tableInfo = this.sqlite
        .prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='sessions'")
        .get() as { sql?: string } | undefined

      if (tableInfo?.sql && tableInfo.sql.includes('user_id')) {
        console.log('⚠️  Detected outdated sessions table schema with user_id column')
        ...
        // Drop old table
        this.sqlite.exec('DROP TABLE IF EXISTS sessions;')
        ...
        // Restore data (excluding user_id column)
        this.sqlite.exec(`
          INSERT INTO sessions (id, project_path, model_id, title, metadata, last_message_at, created_at, updated_at)
          SELECT id, project_path, model_id, title, metadata, last_message_at, created_at, updated_at
          FROM sessions_backup;
        `)

        // Drop backup table
        this.sqlite.exec('DROP TABLE sessions_backup;')

        console.log('✅ Sessions table migration completed')
      }
    } catch (error) {
      console.error('❌ Failed to migrate sessions table:', error)
      // Don't throw - allow app to continue with existing table
    }
  }
  ...
  private migrateMcpServersTable(): void {
    try {
      ...
      // Check if table exists but doesn't have config_json column
      if (tableInfo?.sql && !tableInfo.sql.includes('config_json')) {
        console.log('⚠️  Detected outdated mcp_servers table schema')
        console.log('🔄 Migrating mcp_servers table...')

        // Drop old table (no data to preserve since schema is fundamentally different)
        this.sqlite.exec('DROP TABLE IF EXISTS mcp_servers;')
        ...
        console.log('✅ MCP servers table migration completed')
      }
    } catch (error) {
      console.error('❌ Failed to migrate mcp_servers table:', error)
      // Don't throw - allow app to continue with existing table
    }
  }

  private initTables(): void {
    // Check and migrate tables if they have outdated schema
