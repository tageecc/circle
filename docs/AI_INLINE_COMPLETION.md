...
| 129–179 | `postProcessCompletion`：去 markdown 围栏；去 `</think>`/`<thinking>` 等；去 `<                                                                                                                            | fim\_\* | >`、`< | endoftext | >` 等；`trim`；**去重前缀**：取光标行前 **最后 3 个 token**，若补全以相同前缀开头则剥掉，减轻模型重复输出已有词；超过 20 行则截断。 |
